...
{
	/// <summary>
	/// The OrientedSize structure is used to abstract the growth direction from the layout
	/// algorithms of WrapPanel. When the growth direction is oriented horizontally (ex: the next
	/// element is arranged on the side of the previous element), then the Width grows directly with
	/// the placement of elements and Height grows indirectly with the size of the largest element
	/// in the row. When the orientation is reversed, so is the directional growth with respect to
	/// Width and Height.
	/// </summary>
	...
	internal struct OrientedSize
	{
		...
		/// <summary>
		/// Gets or sets the size dimension that grows indirectly with the maximum value of the
