...
namespace BiliBili.UWP.Controls
{
	public static class CarouselHelper
	{
		...
		public static async Task AnimateXAsync(this FrameworkElement element, double x, double duration = 250, EasingFunctionBase easingFunction = null)
		{
			...
			{
				// await element.Offset(offsetX: (float)element.GetTranslateX(), offsetY: 0,
				// duration: duration, delay: 0, easingType: EasingType.Default).StartAsync();
				// //Offset animation can be awaited
