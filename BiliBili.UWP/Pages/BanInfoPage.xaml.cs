	...
	public sealed partial class BanInfoPage : Page
	{
		...
		private void btn_Download_Click(object sender, RoutedEventArgs e)
		{
			if (gv_Play.Items != null)
			{
				...
				// if (item.IsDowned == Visibility.Collapsed) { DownloadHelper2.CreateDownload(new
				// DownloadTaskModel() { downloadMode = DownloadMode.Anime, sid = _banId, cid =
				// item.danmaku.ToString(), epIndex = item.orderindex, epid = item.episode_id,
				// epTitle = item.index + " " + item.index_title, thumb = (this.DataContext as
				// BangumiDataModel).cover, quality = cb_Qu.SelectedIndex + 1, title =
				// (this.DataContext as BangumiDataModel).title });

				// //var vitem = new PlayerModel() { Aid = _banId, Mid = item.danmaku.ToString(),
				// Mode = PlayMode.Video, No = item.index, VideoTitle = item.index_title, Title =
				// (this.DataContext as BangumiInfoModel).title };

				// //DownloadModel m = new DownloadModel(); //m.folderinfo = new FolderListModel()
				// //{ // id = _banId, // desc = txt_desc.Text, // title = txt_Name.Text, //
				// isbangumi = true, // thumb = (this.DataContext as BangumiInfoModel).cover

				// //}; //m.videoinfo = new VideoListModel() //{ // id = _banId, // mid = vitem.Mid,
				// // part = 1, // partTitle = vitem.No + " " + vitem.VideoTitle, // videoUrl =
				// await ApiHelper.GetBiliUrl_Ban(vitem, cb_Qu.SelectedIndex + 1), // title =
				// txt_Name.Text //};

				// //DownloadHelper.StartDownload(m); Utils.ShowMessageToast("任务已加入下载", 3000); }
				// else { Utils.ShowMessageToast("此视频已下载", 3000); }
