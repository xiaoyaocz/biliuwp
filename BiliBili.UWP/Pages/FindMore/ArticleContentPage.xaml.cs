...
namespace BiliBili.UWP.Pages.FindMore
{
	...
	public sealed partial class ArticleContentPage : Page
	{
		...
		private async void web_NewWindowRequested(WebView sender, WebViewNewWindowRequestedEventArgs args)
		{
			...
			// var md = new MessageDialog("是否调用外部浏览器打开此链接？"); md.Commands.Add(new UICommand("确定",
			// new UICommandInvokedHandler(async (e) => { await
			// Windows.System.Launcher.LaunchUriAsync(args.Uri); }))); md.Commands.Add(new
			// UICommand("取消", new UICommandInvokedHandler((e) => { }))); await md.ShowAsync();
