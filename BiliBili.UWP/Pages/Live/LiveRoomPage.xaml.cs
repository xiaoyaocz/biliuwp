...
namespace BiliBili.UWP.Pages
{
	...
	{
		private BiliLiveDanmu _biliLiveDanmu;
		private int _countClear = 100;
		private string _roomid = "";
		private SystemMediaTransportControls _systemMediaTransportControls;
		private bool DanmuOpen = true;
		...
		private string nowQn = "";

		private bool playUrlloading = false;

		private bool ROUND = false;

		private bool settingloading = true;
		...
		private async void GetPlayUrl(int qn = 0)
		{
			try
			{
				...
				// string results = await wc.GetResults(new
				// Uri("http://live.bilibili.com/api/playurl?platform=h5&cid=" + rommID + "&rnd=" +
				...
				// string playUrl = Regex.Match(results, "
				// <url>(.*?)</url>
				// ").Groups[1].Value; playUrl = playUrl.Replace(" <![CDATA[", ""); playUrl =
				// playUrl.Replace("]]> ", "");
		...
		private async void SendMyGift(string giftId, int Num, string bag_id)
		{
			try
			{
				...
				// Utils.ShowMessageToast("暂时不能赠送礼物", 3000); return;
				pr_Load.Visibility = Visibility.Visible;
