    ...
    public sealed partial class LiveRoomPage : Page
    {
        ...
        private async void GetComment()
        {
            try
            {
                ...
                string url = string.Format("http://live.bilibili.com/AppRoom/msg?_device=wp&appkey={0}&build=5250000&access_key={1}&platform=android&room_id={2}&ts={3}", ApiHelper._appKey, ApiHelper.access_key, _roomid, ApiHelper.GetTimeSpan);
                url += "&sign=" + ApiHelper.GetSign(url);
                string results = await WebClientClass.GetResults(new Uri(url));
        ...
        private async void cb_Source_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
        ...
        private async void GetGiftTop(string room_id)
        {
            try
            {
                ...
                string url = string.Format("http://live.bilibili.com/AppRoom/getGiftTop?_device=wp&appkey={0}&build=5250000&access_key={1}&platform=android&room_id={2}&ts={3}", ApiHelper._appKey, ApiHelper.access_key, room_id, ApiHelper.GetTimeSpan);
                url += "&sign=" + ApiHelper.GetSign(url);
                string results = await WebClientClass.GetResults(new Uri(url));
        ...
        private async void GetFansTop(string room_id)
        {
            ...
            {
                list_Fans_Top.Items.Clear();
                string url = string.Format("http://live.bilibili.com/AppRoom/medalRankList?_device=wp&appkey={0}&build=5250000&access_key={1}&platform=android&room_id={2}&ts={3}", ApiHelper._appKey, ApiHelper.access_key, room_id, ApiHelper.GetTimeSpan);
                url += "&sign=" + ApiHelper.GetSign(url);
                string results = await WebClientClass.GetResults(new Uri(url));
                LiveRankModel model = JsonConvert.DeserializeObject<LiveRankModel>(results);
                if (model.code == 0)
        ...
        private async void SendMyGift(string giftId, int Num, string bag_id)
        {
            try
            {
                ...
                string url = string.Format("http://live.bilibili.com/AppBag/send?access_key={0}&appkey={1}&build=522000&mobi_app=android&platform=android", ApiHelper.access_key, ApiHelper._appKey);
                url += "&sign=" + ApiHelper.GetSign_Android(url);
        ...
        private async void SendBuyGift(string giftid, int num)
        {
            try
            {
                ...
                string url = string.Format("http://live.bilibili.com/mobile/sendGift?_hwid={0}&coinType={1}&giftId={2}&num={3}&rnd={4}&roomid={5}&ruid={6}&appkey={7}",
                    ApiHelper._buvid, type, giftid, num, new Random().Next(10000, 99999) + new Random().Next(10000, 99999), _roomid, (Video_UP.DataContext as LiveInfoModel).mid, ApiHelper._appKey);
                url += "&sign=" + ApiHelper.GetSign(url);
                string results = await WebClientClass.GetResults(new Uri(url));
                JObject m = JObject.Parse(results);
                if ((int)m["code"] == 0)
